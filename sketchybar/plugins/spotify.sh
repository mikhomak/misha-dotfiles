...
update_track() {
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}" label.drawing=yes icon.color=0xffa6da95 background.color=$WIN98_BRIGHT_GREY background.shadow.angle=30

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME icon.color=0xffeed49f background.color=$WIN98_GREY background.shadow.angle=220
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME icon.color=0xffeed49f label.drawing=no background.color=$WIN98_GREY background.shadow.angle=220
    else
        sketchybar --set $NAME icon.color=0xffeed49f background.color=$WIN98_GREY background.shadow.angle=220
    fi
}
