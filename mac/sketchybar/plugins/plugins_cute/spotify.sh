...
update_track() {
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}" label.drawing=yes icon.color=0xffa6da95 background.color=$BLUE_LIGHT background.shadow.angle=270 background.shadow.distance=15

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME icon.color=0xffa6da95 background.color=$PINK_MIDLE background.shadow.angle=90 background.shadow.distance=1
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME icon.color=0xffa6da95 label.drawing=no background.color=$BLUE_LIGHT background.shadow.angle=90 background.shadow.distance=1
    else
        sketchybar --set $NAME icon.color=0xffa6da95 background.color=$PINK_LIGHT background.shadow.angle=90 background.shadow.distance=1
    fi
}
